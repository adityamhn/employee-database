...
const Card = ({name,email,id,job_title,DOB,department,onRouteChange,cardInfo}) => {
	...
	return (
      <div className="tc system-sans-serif bg-lightest-blue dib br2 pa3 ma2 grow bw2 shadow-5 w5">
            ...
            cardInfo(name,email,id,job_title,DOB,department)
      }
      } 
            icon={faUserEdit} />
      ...
      <p className="ttc mt2 mb0 gray">{job_title}</p>
      <p className="ttc mt0 gray">{department}</p>
      </div>
      </div>
		);
}
